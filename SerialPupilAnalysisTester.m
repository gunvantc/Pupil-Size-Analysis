...
A_VID_NAME = 'AudDistract_W12_020817';
%A_EYE_CENTER = [];
A_OFFSET_THRESH = 0.105;
%A_CROP_RECT = [];
A_FILTER = 1;
A_FILTER_ORDER = 3;
...
%IF YOU WANT TO SPEED UP ANALYSIS, CHANGE TO <1
scale_factor = 1;
...
if exist('eye_center_orig')==0
    ...
    single_frame_no_crop = read(video,100);
...
%TOGGLE VALUE TO SWITCH BETWEEN SETUP AND RUN MODES
SETUP = 1;
...
for i = 40000:FRAME_STEP:(number_of_frames+FRAME_STEP-1)
    j=i+FRAME_STEP-1;
    ...
    if A_FILTER == 1
        bandStop_frames = DC_bandpass_modGC(loadFrames,4,9,.95,0,'myvid','none',30,'low',A_FILTER_ORDER,0);
    else
